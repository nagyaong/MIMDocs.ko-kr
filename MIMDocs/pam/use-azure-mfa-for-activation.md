...
- 역할 소유자 승인
- [Azure MFA(Multi-Factor Authentication)](https://docs.microsoft.com/azure/multi-factor-authentication/multi-factor-authentication)
...
3.  Windows 탐색기를 사용하여 이전 섹션에서 다운로드한 ZIP 파일의 ```pf\certs``` 폴더로 이동합니다. ```cert\_key.p12``` 파일을 새 디렉터리에 복사합니다.

4.  Windows 탐색기를 사용하여 ZIP 파일의 ```pf``` 폴더로 이동하고 워드패드와 같은 텍스트 편집기에서 ```pf\_auth.cs``` 파일을 엽니다.

5. 다음 세 매개 변수를 찾습니다. ```LICENSE\_KEY```, ```GROUP\_KEY```, ```CERT\_PASSWORD```
...
- [오늘 Azure 계정 만들기](https://azure.microsoft.com/free/)