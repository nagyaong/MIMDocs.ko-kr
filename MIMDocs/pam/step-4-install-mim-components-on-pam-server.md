...
8.  SharePoint 사이트 모음 URL을 *http://pamsrv.priv.contoso.local:82*로 설정합니다.
...
12. SharePoint와 동일한 계정을 사용하도록 MIM PAM REST API 계정을 구성합니다(MIM 포털이 이 서버에 함께 있을 때).
    ...
    - 응용 프로그램 풀 계정 암호: *Pass@word1*(또는 2단계에서 만든 암호)  
...
5.  샘플 웹 응용 프로그램을 구성합니다. 메모장과 같은 텍스트 편집기를 사용하여 **C:\Program Files\Microsoft Forefront Identity Manager\2010\Privileged Access Management Portal\js\utils.js** 파일을 편집합니다. **pamRespApiUrl** 값을 *http://pamsrv.priv.contoso.local:8086/api/pamresources/*로 설정합니다.
